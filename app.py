...
location_df = location_df.join(df_cases, how="inner")
...
location_df["id"] = location_df["country_name"].map(lambda x: country_info[country_info["name"] == x]["numericCode"].values[0])
countries = alt.topo_feature(data.world_110m.url, 'countries')


points = alt.Chart(countries).mark_geoshape(stroke='black')\
